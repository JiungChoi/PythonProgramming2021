...
pygame.display.set_caption("dmTest")

clock = pygame.time.Clock()

total_score = 0 #점수
level_control = 1 #난이도??
total_level = 0 #난이도를 맞추기위한 점수
total_level_list = [0, 5, 10, 15, 20,20,20,20,20,20,20,20,20]

game_font = pygame.font.Font('C:/WINDOWS/Fonts/ARIBLK.TTF', 20) #나중에 바꿀 폰트

background = pygame.image.load("back.png")  #배경사진
...
class enemy_class:
    ...
    def __init__(self):
        self.enemy_speed = random.choice([0.2,0.5])
        self.enemy_spawnPoint = random.choice(['UP', 'DOWN', 'LEFT', 'RIGHT'])

        # 스폰 지점 설정
        if self.enemy_spawnPoint == 'LEFT':
            self.enemy_x_pos = random.randint(0, screen_height - self.enemy_height)
