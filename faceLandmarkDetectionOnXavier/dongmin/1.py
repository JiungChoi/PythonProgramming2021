...
class FloatElement: # 부유물 클래스: 점수 클래스(plus)와, 실점(minus) 클래스로 상속한다. 
    ...
    def __init__(self,Floatter_IMG_PATH, w, h):
        ...
        if Floatter_IMG_PATH == "Images/apple.png":
            ...
            self.type = "goldApple"
        elif Floatter_IMG_PATH == "Images/poisonMushroom.png":
            self.type = "poisonMushroom"
...
class PlusElement(FloatElement): # 점수 클래스(plus) : 부유물 객체로 부터 상속 받는다.
    ...
    def eat(self):
        if self.type == "medicine.png":
            ...
            game.score.upScore(SCORE_APPLE) if PlusElement.DOUBLE_MODE == False else game.score.upScore(2*SCORE_APPLE)
        elif self.type == "beef":
...
class Game:
    ...
    def gameFactor(self): # 이미지 파일들의 정보를 관리 : [경로, [x,y], (width, height))]
        ...
        ## Game의 부유물 객체 관리
        self.run_plusElementInfo = [["Images/apple.png", "Images/beef.png", "Images/goldApple.png"], [0, 0], (RUN_BUTTON_SMALL_WIDTH, RUN_BUTTON_SMALL_HEIGHT)]
        self.run_minusElementInfo = [["Images/cucumber.png","Images/poisonMushroom.png"], [0, 0] , (RUN_BUTTON_SMALL_WIDTH, RUN_BUTTON_SMALL_HEIGHT)]
    ...
    def gameStart(self):
        ...
        for points in fc.run(visualize=1, max_threads=4, capture=0):
            ...
            if PlusElement.DOUBLE_MODE:
                print("여기")
                game.SCREEN.blit(game.imgs_run_fire, [game.points[16][0]*1.25,game.points[16][1]*1.1])
                game.SCREEN.blit(game.imgs_run_fire, [game.points[20][0]*1.25,game.points[20][1]*1.1])
